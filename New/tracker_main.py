...
class improcess():
    ...
    ########################      new       functions      new       #############################
    #########################################################################################
    def _pointLaserTo(angles):
        bus.write_byte(addr, angles[0])
        bus.write_byte(addr, angles[1])

    #########################################################################################
    ########################      new        main code      new       #############################
    #########################################################################################

    def runDrill(anglesList):
        print("[Drill] runDrill start")
        for anlge in anglesList:
            print("[Drill] angles : ", anlge[0], " ", anlge[1])
            #turn the servors
            _pointLaserTo(angle)
...
# dete_center = 0
# 
# bus.write_byte(addr, 67)
# bus.write_byte(addr, 69)
# 
...
#                     print("OFF")
#                     #line_string += "Detect"
#                     bus.write_byte(addr, 68)
#                     bus.write_byte(addr, 70)
#                         
#             else:
#                 count = 0
#             
